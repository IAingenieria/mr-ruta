...
export function GET() {
  ...
  const L: string[] = [];
  L.push("# Mr Ruta", "", `> Mr Ruta es una plataforma de reparto y venta en ruta para distribuidoras con flota propia en México. Tres apps en un solo sistema: Radar (encuentra negocios nuevos cerca de la ruta, verificados uno por uno), app del vendedor (pedido sugerido, devolución y cobro en una pantalla, funciona sin señal) y despacho (pedidos desde Excel, Odoo o Microsip, ruta ordenada, evidencia de entrega con foto, GPS, hora y firma). Cada empresa tiene su propia instancia. Hecha en México por ${SITIO.empresa}.`, "", `Sitio: ${d}`, `Demo guiada con tu giro y tu ciudad en 30 segundos: ${d}/demo`, "");
